...
/// <summary>
/// Each of the different sets of movement permissions that a weapon might allow during one of its firing states.
/// </summary>
...
/// <summary>
/// Each of the states a weapon cycles through in the process of firing and refiring.
/// </summary>
...
/// <summary>
/// Static class containing data used by PlayerWeapon, plus
/// the "plug-in" functions that actually handle weapon-specific logic.
/// </summary>
static class PlayerWeaponStatic
{
    ...
    /// <summary>
    /// Call this if you need to use the AudioClip arrays, but they're
    /// not initialized yet.
    /// </summary>
    ...
    /// <summary>
    /// Recoil function for shotgun-type weapons.
    /// </summary>
...
/// <summary>
/// Models a player weapon. There are two of these attached to the Player GameObject -
/// one per weapon slot.
/// </summary>
public class PlayerWeapon : MonoBehaviour
{
    ...
    /// <summary>
    /// Called every frame.
    /// Note: PlayerWeapon is slaved to PlayerController, and anything that actually relies on player input
    /// has to go through the ReceiveInput() method that PlayerController calls.
    /// </summary>
    void Update ()
    {
    ...
    /// <summary>
    /// Receives input. Called by PlayerController, but only if we're allowed
    /// to get fire button inputs that frame. This is brainless - assumes that
    /// we're allowed to look at the button if it's called!
    /// </summary>
    ...
    public void ReceiveInput (VirtualButton _btn)
    {
        ...
        switch (state)
        {
            case WeaponState.Ready:
                if (btn.Pressed && manager.master.animator.GetBool(PlayerAnimatorHashes.paramFiringAllowed))
                {
                    if (wpnTypeBuffer != WeaponType.None)
                    {
                ...
                else if (manager.master.animator.GetBool(PlayerAnimatorHashes.paramNowFiring) == true)
                {
                    if (isWpn2 == true)
                    {
                        if (manager.master.wpn1.state == WeaponState.Ready)
                        {
                            manager.master.animator.SetBool(PlayerAnimatorHashes.paramNowFiring, false);
                        }
                    }
                    else if (manager.master.wpn2.state == WeaponState.Ready)
                    {
                        manager.master.animator.SetBool(PlayerAnimatorHashes.paramNowFiring, false);
                    }
                }
    ...
    /// <summary>
    /// Handles player sprite movement while firing.
    /// TO-DO: implement FullMobility and WalkingOnly.
    /// </summary>
    ...
    /// <summary>
    /// Resets temporary weapon state data.
    /// </summary>
    public void Flush ()
    {
        state = WeaponState.Ready;
    ...
    /// <summary>
    /// Loads weapon actions and mobility types corresponding to the weapon type in wpnTypeBuffer.
    /// </summary>
    void ReloadActions ()
    {
        Flush();
